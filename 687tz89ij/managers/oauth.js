...
module.exports = (app) => {
	...
	app.post('/account/api/oauth/token', async (req, res) => {
		var displayName = "Variance";
		var accountId = "Variance";
		...
		Profile.updatedCos(profileData);
		res.json({
	...
	app.get('/account/api/public/account/', (req, res) => {
		try {
			...
			if (`${req.query.accountId}`.startsWith("NeoRiftBot")) { displayName = "NeoRiftBot" }
			...
			req.query.accountId.forEach(accId => {
				var dn = accId
				if (`${accId}`.startsWith("NeoRiftBot")) { dn = "NeoRiftBot" }
				response.push({
					id: accId,
					displayName: dn,
					externalAuths: {}
